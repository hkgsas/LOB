...
def _parse_opts():
    ...
    if arg.device is None:
        arg.device = [0] if torch.cuda.is_available() else 'cpu'
        arg.cuda = True if torch.cuda.is_available() else False
    else:
        if isinstance(arg.device, Iterable) is False:
            arg.device = [arg.device]
        arg.device = list(arg.device) if torch.cuda.is_available() else 'cpu'
        arg.cuda = True if torch.cuda.is_available() else False

    arg.num_gpus= len(arg.device)
